...
router.post('/:id/join/', async (req, res) => {
	...
	const firstValidTeam = await db.collection('teams').where('league', '==', id).where('owner', '==', 'default').limit(1).get();
	let teamData = null;
	if (firstValidTeam.empty) {
		...
		admin.auth().getUserByEmail(owner).then(async user => {
			await db.collection('teams').doc(teamData.id).update({'owner' : user.uid, 'ownerName' : owner});
