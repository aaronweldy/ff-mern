...
router.post("/:leagueId/runScores/", requireAuth, async (req, res, next) => {
    try {
        ...
        if (Object.values(data).some((player) => !Number.isFinite(player.scoring.totalPoints))) {
            throw new Error("Scoring data contains invalid points; scores were not saved.");
        }
        ...
        const result = await db.runTransaction(async (transaction) => {
            ...
            transaction.set(db
                .collection("leagueScoringData")
                .doc(`${getCurrentSeason()}${week}${leagueId}`), { playerData: data });
            transaction.update(leagueRef, {
                lastScoredWeek: Math.max(week, leagueSnapshot.data()?.lastScoredWeek || 0),
            });
            return { teams, errors };
        });
