...
export const calculatePlayerScore = (statistics, position, scoringSettings) => {
    const categories = scoringSettings
        .filter((setting) => setting.position.includes(position))
        .map((setting) => {
        const category = setting.category;
        const hash = category.qualifier === "between"
        ...
        try {
            ...
            const statNumber = Number.parseFloat(statistics[statKey]);
            if (Number.isNaN(statNumber))
                return { [hash]: 0 };
            let points = 0;
            switch (category.qualifier) {
                case "per":
                    ...
                    break;
                case "greater than":
                    if (statNumber >= category.threshold)
                        points = setting.points;
                    break;
                case "between":
                    if (statNumber >= (category.thresholdMin || Infinity) &&
            ...
            const minimumsPass = setting.minimums.every((minimum) => {
                ...
                return (Number.parseFloat(statistics[statKey]) >= minimum.threshold);
            });
    ...
    return {
        totalPoints: Number.parseFloat(categories
