...
export const calculateCumulativeScores = (previous, week, data) => {
    const result = JSON.parse(JSON.stringify(previous));
    const knownNames = new Map(Object.keys(result).map((name) => [sanitizePlayerName(name), name]));
    for (const [key, stats] of Object.entries(data)) {
        const name = knownNames.get(key) ||
        ...
        entry.totalPointsInSeason = entry.pointsByWeek.reduce((sum, points) => sum + points, 0);
        result[name] = entry;
    }
    return result;
};
//# sourceMappingURL=cumulativeScoring.js.map