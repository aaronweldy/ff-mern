import { AbbreviationToFullTeam, playerTeamIsNflAbbreviation, sanitizePlayerName, singlePositionTypes, } from "@ff-mern/ff-types";
 ...
 */
export const fetchWithRetry = async (url, init = {}, retries = MAX_RETRIES, backoffMs = INITIAL_BACKOFF_MS) => {
    let lastError;
    for (let attempt = 1; attempt <= retries; attempt++) {
        try {
            ...
            console.warn(`Fetch attempt ${attempt}/${retries} for ${url} failed: ${err}. ` +
 ...
 */
export const fetchHtmlWithRetry = async (url) => {
    const response = await fetchWithRetry(url, {
        headers: {
            "User-Agent": "Mozilla/5.0 (compatible; orca-ff-functions/1.0; +https://firebase.google.com)",
...
export const parseHtmlTables = (html) => {
    ...
    $("table").each((_, table) => {
        ...
        const $headerRow = $table.find("thead tr").first().length > 0
 ...
 */
export const hasFetchedRecently = async (docId) => {
    try {
        ...
        const lastDate = typeof lastFetched.toDate === "function"
            ? lastFetched.toDate()
 ...
 * @return {Promise<void>}
 */
export const markFetched = async (docId) => {
    await db
...
const fetchTeamDefensePerformance = async () => {
    ...
    for (const { pos, nflPosition } of positions) {
        try {
            const url = `https://fantasy.nfl.com/research/pointsagainst?position=${nflPosition}` +
                `&statCategory=stats&statSeason=2024&statType=seasonStats`;
            const data = await get(url);
            if (data[0]) {
                data[0].forEach((teamData) => {
...
async function fetchAndParseESPNSchedule(schedule, week) {
    const url = `http://site.api.espn.com/apis/site/v2/sports/football/nfl/` +
...
async function updateFirebase(schedule) {
...
async function updateScheduleForAllWeeks(startWeek, endWeek) {
    const schedule = {};
...
const fetchSeasonProjections = async () => {
    ...
    const overallData = (overallTables[0] ?? []);
        ...
        try {
            ...
            for (const playerData of data) {
                if (playerData["Player Team (Bye)"]) {
                    const parsedData = parsePlayerFromScrapedData(playerData["Player Team (Bye)"]);
                    if (parsedData) {
...
export const fetchNflSchedule = onSchedule("every day 00:00", async () => {
    ...
    try {
        ...
        for (let i = 2; i < data.length; i++) {
            dbUpdate[AbbreviationToFullTeam[data[i]["0"]]] = data[i];
        }
...
export const runScoresForAllLeagues = onSchedule("0 2,22 * * *", async () => {
    ...
    for (const league of allLeagues.docs) {
        ...
        try {
            ...
            console.error(`runScores failed for league ${leagueId} after ${MAX_RETRIES} ` +
                `attempts (backend may be down), continuing:`, err);
            continue;
        }
...
const getWeekFromPuppeteer = async () => {
    try {
        const html = await fetchHtmlWithRetry("https://www.fantasypros.com/nfl/stats/qb.php?range=week");
        const $ = load(html);
...
export const fetchLatestFantasyProsScoredWeek = onRequest({ timeoutSeconds: 60 }, async (req, res) => {
    try {
